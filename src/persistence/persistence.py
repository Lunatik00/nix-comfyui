    ...
    try:
        ...
        if app_dir:
            ...
            if not os.path.exists(patch_file):
                ...
                custom_node_patch = os.path.join(app_dir, 'custom_nodes', 'model_downloader', 'model_downloader_patch.py')
                if os.path.exists(custom_node_patch):
                    ...
                    try:
                        ...
                        logger.info(f"Created symlink for model_downloader_patch.py: {custom_node_patch} -> {patch_file}")
                    except Exception as e:
                        # If symlink fails, try to copy the file
                        import shutil
                        shutil.copy2(custom_node_patch, patch_file)
                        logger.info(f"Copied model_downloader_patch.py to app directory: {custom_node_patch} -> {patch_file}")
        ...
        logger.error(f"Error preparing model downloader: {e}")
...
def patch_folder_paths(base_dir):
    ...
    try:
        ...
        logger.error(f"Error patching folder_paths: {e}")

def setup_persistence():
    """Set up the persistence for ComfyUI by creating symlinks between the app directory and persistent storage"""
    ...
    for model_dir in model_dirs:
        ...
        try:
            ...
            logger.error(f"Error creating symlink for {model_dir}: {e}")
    ...
    for user_dir in user_dirs:
        ...
        try:
            ...
            logger.error(f"Error creating symlink for {user_dir}: {e}")
    ...
    try:
        ...
        logger.error(f"Error patching model downloader: {e}")
